...
public class InsXML2DB{

	// Constructor
	public InsXML2DB () throws Exception {
	}
	...
	public static void main (String argv[]) throws Exception {
		...
		if (argv.length > 0)
			...
			//fileXML="test.xml";}
			fileXML="test.xml";}
		InsXML2DB insXML2DB = new InsXML2DB();

		
		insXML2DB.bacaDataXML(fileXML);
	...
	try {
		...
		for(int s=0; s<listOfTransactions.getLength() ; s++){
			...
			if(firstPersonNode.getNodeType() == Node.ELEMENT_NODE){
				...
				bacaNode(firstPersonElement, "time").substring(0,2)+":"+
				bacaNode(firstPersonElement, "time").substring(2,4))+"'";
				String n3="'"+(
				bacaNode(firstPersonElement, "date")+" "+
				...
				for(int p=0; p<listOfItems.getLength() ; p++){
					...
					if(firstItemNode.getNodeType() == Node.ELEMENT_NODE){
						...
						String insval=(n1+","+n2+","+n3+","+n4+","+n5+","+n6+","+n7+","+n8+","+n9+","+n10+","+n11+",'',"+n13+","+n14+","+n15+","+n16+","+n17+","+n18+","+n19+","+n20+","+n21+","+n22+","+n23+","+n24+","+n25+",'','','','','',''");
						System.out.println(insval);
						db.update("INSERT INTO Firmansyah.temp_STORE_TRAN_DTL VALUES(" + insval + ")");
